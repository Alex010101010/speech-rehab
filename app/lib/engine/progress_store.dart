...
/// показа ("yyyy-mm-dd"). Заводится на задания, решённые верно — сам или с
/// подсказкой (см. [ReviewScheduler.seeds]).
class ReviewCard {
...
/// Повторяем успешно извлечённое (retrieval practice) — сам или с подсказкой,
/// но не неуспешное (показан ответ): errorless-повтор проигрывает на отложенных
/// замерах и бьёт по мотивации (см. ревью афазия-приложений 28.06, PMC10023178).
/// Полная лесенка: «сегодня» (повтор внутри этой же сессии, см. блок «Закрепим»)
/// → 1д → 2д → 5д → 14д → 30д. Здесь — только межсессионная часть (от 1д);
/// «сегодня» живёт в сессии бонусом и в расписание не пишется.
class ReviewScheduler {
  // дни до следующего показа по стадии («коробке»)
  static const intervals = <int>[1, 2, 5, 14, 30];

  /// Заводить ли карточку на задание после основного шага. Решённое с
  /// подсказкой тоже засеваем — оно-то и нуждается в закреплении. Стартует с
  /// той же коробки, но на повторе «вспомнил» засчитывается только без
  /// подсказки, так что такое слово возвращается каждый день, пока не
  /// вспомнится само. Показанный ответ и самооценка — не засеваем.
  static bool seeds(StepOutcome o) => o.gradeable && o.correct;
